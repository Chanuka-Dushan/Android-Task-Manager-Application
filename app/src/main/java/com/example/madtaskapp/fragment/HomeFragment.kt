...
import com.example.madtaskapp.adapter.TaskAdapter
import com.example.madtaskapp.databinding.FragmentHomeBinding
...
class HomeFragment : Fragment(R.layout.fragment_home),SearchView.OnQueryTextListener,MenuProvider {
    ...
    private fun setupHomeRecyclerView(){
        ...
        activity?.let {
            taskViewModel.getAllTasks().observe(viewLifecycleOwner){task->taskAdapter.differ.submitList(task)
            updateUI(task)
            }
        }
    }
    ...
    private fun searchTask(query: String?){
        ...
        taskViewModel.searchTask(searchQuery).observe(this){
            list->taskAdapter.differ.submitList(list)
        }
    }
